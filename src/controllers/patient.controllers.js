...
import { patientZodSchema, patientPartialZodSchema } from "../validators/patient.validator.js";
...
const getPatientByPN = asyncHandler(async(req, res, next) => {
  ...
  res.status(200).json(new ApiResponse(200, "Patient retrieved successfully", patient));
})
...
const deletePatient = asyncHandler(async (req, res, next) => {
  ...
  res.status(200).json(new ApiResponse(200, "Patient deleted successfully", deletedPatient));
});

export { getPatients, createPatient, getPatientByPN, replacePatient, updatePatient, deletePatient };